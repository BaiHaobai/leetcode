...
class Solution(object):
    def getStrongest(self, arr, k):
        ...
        """
        def cmp(x,y):
            if abs(x-self.m)>abs(y-self.m):
                return 1
            elif abs(x-self.m)==abs(y-self.m):
                if x>y:
                    return 1
                else:
                    return -1
                ...
                return -1
        self.m=sorted(arr)[(len(arr)-1)/2]
        #print self.m
        #print sorted(arr,cmp=cmp,reverse=True)
        return sorted(arr,cmp=cmp,reverse=True)[:k]
```
JavaScript Code:
