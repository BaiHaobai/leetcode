    ...
    def updateBoard(self, board, click):
        ...
        for i in range(rows):
            for j in range(columns):
                if board[i][j]=='M':
                    a[i][j]='M'
                    if i-1>=0 and j-1>=0 and a[i-1][j-1]!='M':
                        a[i-1][j-1]+=1
                    if i-1>=0 and a[i-1][j]!='M':
                        a[i-1][j]+=1
                    if j-1>=0 and a[i][j-1]!='M':
                        a[i][j-1]+=1
                    if i+1<rows and j+1<columns and a[i+1][j+1]!='M':
                        a[i+1][j+1]+=1
                    if i+1<rows and a[i+1][j]!='M':
                        a[i+1][j]+=1
                    if j+1<columns and a[i][j+1]!='M':
                        a[i][j+1]+=1
                    if i+1<rows and j-1>=0 and a[i+1][j-1]!='M':
                        a[i+1][j-1]+=1
                    if i-1>=0 and j+1<columns and a[i-1][j+1]!='M':
                        a[i-1][j+1]+=1
        def mi(i,j):
